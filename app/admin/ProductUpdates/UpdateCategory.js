...
const UpdateCategoryPopup = ({ setUpdateCategory, categoryToUpdate, setCategoryToUpdate }) => {
    ...
    const handleUpdateCategory = async () => {
        try {
            ...
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_BACKEND}/pcategory3/update/${categoryToUpdate._id}`,
                {
                    method: "PUT",
                    headers: {
            ...
            if (!response.ok) {
                return toast.error(data.message || "Failed to update category.");
            }

            toast.success("Category updated successfully!");
            setUpdateCategory(false); // Close the popup
        } catch (error) {
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-purple-300 bg-opacity-50">
            <div className="bg-white p-6 rounded-lg shadow-lg w-80 sm:w-[40vw] md:w-[50vw] lg:w-[60vw]">
                <h3 className="text-lg font-bold mb-4">
                    Update Category: {categoryToUpdate?.name || "Unknown"}
                </h3>
