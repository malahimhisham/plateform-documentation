...
const CategorySession = ({ category, setIsSubCategory }) => {
    ...
    const fetchSessions = async () => {
        ...
        try {
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND}/pcategorysession1/search/${category._id}`, {
                headers: {
    ...
    const moveSession = async (draggedId, targetId) => {
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND}/pcategorysession1/update-order`, {
                method: 'PUT',
                headers: {
